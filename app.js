const ranks = { member: 'Community Member', support: 'Support Team', management: 'Management', developer: 'Developer', leaddev: 'Lead Developer', coreteam: 'Core Team' };
...
async function reviewApplication(id, decision) {
  ...
  if (decision === 'accepted') {
    try {
      const response = await fetch('/api/applications/review', { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'same-origin', body: JSON.stringify({ application, decision }) });
...
function roleBadge(role) { const style = roleStyles[role] || roleStyles.member; return `<span class="role-badge role-${role}" style="--role-color:${style.color};--role-bg:${style.bg}">${escape(ranks[role] || role)}</span>`; }
...
function setRole(role) {
  ...
  $('#roleName').textContent = ranks[role]; $('#staffNav').style.display = state.authenticated && isStaff() ? 'block' : 'none';
...
function login() { window.location.assign('/api/auth/discord'); }
...
async function syncAuth() { try { const response = await fetch('/api/auth/me'); if (!response.ok) return; const { user } = await response.json(); state.authenticated = true; state.user = user; $('#accountButton b').textContent = user.username; $('#accountButton .avatar').innerHTML = `<img src="${avatarUrl(user.id, user.avatar)}" alt="">`; setRole(user.role); renderTickets(); renderNotifications(); updateNotificationBadge(); } catch {} }
...
function renderQueue() {
  ...
  $$('[data-delete-ticket]').forEach(button => button.addEventListener('click', async () => {
    ...
    try {
      const response = await fetch('/api/tickets/transcript', { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'same-origin', body: JSON.stringify({ ticket }) });
      const result = response.status === 204 ? {} : await response.json();
...
async function loadOnlineStaff() {
  try {
    ...
    $('#onlineStaff').innerHTML = staff.length
      ? staff.map(person => `<div><div class="staff-avatar blue">${person.avatar ? `<img src="${avatarUrl(person.id, person.avatar)}" alt="">` : escape(person.name[0])}</div><span><b>${escape(person.name)}</b><small>${ranks[person.role] || person.role}</small></span><i title="${escape(person.status || 'online')}">•</i></div>`).join('')
